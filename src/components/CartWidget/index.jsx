...
class CartWidget extends React.Component {
	...
	renderItems() {
		const { cart, removeFromCart, updateAmount } = this.props;

		if (cart && cart.length !== 0) {
			return cart.map((item, i) => (
				<div key={i} className="cart-widget-item">
					<div className="title">
		...
		return (
			<div className="empty">
				<p className="message">Ваша корзина пока пуста</p>
				<Link to="/shop" onClick={this.handleExpand}>
					<Button inverted basic fluid>
						Выбрать товар
					</Button>
				</Link>
			</div>
	...
	renderTotal() {
		const { total, totalWithDiscount } = this.props;

		if (totalWithDiscount) {
			return (
				<span className="cart-total">
					<span className="lined-throught">{total}</span>
					<span className="total-to-checkout">{totalWithDiscount}</span>
					<span className="suffix">руб.</span>
				</span>
			);
		}
		...
		return (
			<span className="cart-total">
				<span className="total-to-checkout">{total}</span>
				<span className="suffix">руб.</span>
			</span>
		);
	}
	...
	renderButton() {
		return (
			<div className="cart-checkout-button">
				<div className="total">Итого: {this.renderTotal()}</div>
				<Link onClick={this.handleExpand} to="/shop/cart">
					<Button fluid basic>
	...
	render() {
		const { isExpanded, animation } = this.state;

		const { cart, total, promo } = this.props;
			...
			return (
				<div className="cart-widget-cart">
					<div className="info">
						...
						<div className="amount">{cart.length} в корзине</div>
					</div>
					<Button
						...
						icon
					>
						<Icon name={!isExpanded ? "angle down" : "angle up"} /> Моя корзина
					</Button>
					{isExpanded && (
						<div className="expand">
							...
							{cart.length > 0 && (
								<div className="bottom">
									<div className="checkout">{this.renderButton()}</div>
									{!promo && (
										<div className="promo">
											<PromoInput />
										</div>
