...
const { errCode0009, errCode0010 } = require("../../errors");
...
router.post(
	...
	({ matchedData }, res) =>
		Promo.create(matchedData)
			...
			.catch(
				error => console.log(error) || res.status(500).json(errCode0009(error))
			)
);
...
router.get("/check", (req, res) =>
	Promo.findOne({
		...
		.catch(
			error => console.log(error) || res.status(500).json(errCode0010(error))
		)
);
