...
router.post(
	"/register",
	[validateRegister, handleValidation, saltPassword],
	({ matchedData }, res) =>
		Admin.create(matchedData)
			.then(user => res.json(user))
			.catch(err => res.status(500).json(errCode0005(err)))
);

router.post("/auth", [validateAuth, handleValidation], ({ matchedData }, res) =>
	Admin.findOne({ [ADMIN_LOGIN]: matchedData[ADMIN_LOGIN] })
		.select(`+${ADMIN_PASSWORD}`)
		.then(admin => {
			if (admin) {
				return password(
					matchedData[ADMIN_PASSWORD] + process.env.SECRET
				).verifyAgainst(admin[ADMIN_PASSWORD], (err, verified) => {
					if (err) {
						return res.status(500).send(errCode0004(err));
					}
					if (!verified) {
						return res.status(500).send(errCode0015());
					}
...
router.get("/check", [auth], ({ user }, res) =>
	res.status(200).json({
		isAuthed: !!user
	})
);
